...
/**
 * Single source of truth for the localStorage prefix. The no-flash script in
 * `__root.tsx`, `useTheme`, the sidebar and the chat widget all read/write
 * `miow-*` keys, so settings must use the same namespace or saved preferences
 * are silently ignored on reload.
 */
